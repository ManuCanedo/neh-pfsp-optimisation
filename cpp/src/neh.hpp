...
public:
  Matrix(size_t rows, size_t cols)
      : data_(rows, std::vector<NumericType>(cols)), cols_(cols) {}

  NumericType &operator()(size_t i, size_t j) {
    return const_cast<NumericType &>(
        const_cast<const Matrix<NumericType> &>(*this)(i, j));
  }
...
constexpr void populate_q_mat(const std::vector<Job<NumericType>> &jobs,
  ...
  for (ssize_t j = q_mat.width() - 2; j != -1; --j) {
    q_mat(index - 1, j) =
...
template <typename NumericType>
constexpr auto try_shift_improve(Solution<NumericType> &solution, size_t index,
  ...
  if (best_index != index) {
    ...
    for (size_t j = index; j != best_index; --j) {
...
template <typename NumericType>
constexpr auto solve(std::vector<Job<NumericType>> &&jobs, size_t number_jobs,
                     size_t number_machines) {
  ...
  for (size_t i = 0; i != solution.number_jobs; ++i) {
    for (size_t j = 0; j != t_mat.width(); ++j) {
      t_mat(i, j) =
          ...
          max(i == 0 ? 0 : t_mat(i - 1, j), j == 0 ? 0 : t_mat(i, j - 1));
    }
  }
  return t_mat(solution.number_jobs - 1, solution.number_machines - 1);
}
} // namespace neh
